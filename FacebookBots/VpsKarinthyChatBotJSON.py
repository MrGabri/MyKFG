...
def setMessage(_lesson, _subs):
    if _subs == False:
        if not _lesson["start"] == "":
            if "+" in _lesson["start"]:
                msg = ("{} a {} teremben {} -val/vel ami T{} percel kesobb kezdodik!".format(_lesson["name"], _lesson["room"],_lesson["teacher"], _lesson["start"])).encode('utf-8')
            if "-" in _lesson["start"]:
                msg = ("{} a {} teremben {} -val/vel ami T{} percel elobb kezdodik!".format(_lesson["name"], _lesson["room"],_lesson["teacher"], _lesson["start"])).encode('utf-8')
        else:
...
while True:
    ...
    if day == "Saturday" or day == "Sunday":
        ...
        if now.hour <= 7:
            ...
            if now.minute == 45:
                try:
                    Lesson = Schedule[day][now.hour - 6]
